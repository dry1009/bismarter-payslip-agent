...
const ChatMessage = memo(({
  ...
  return <div className={cn("flex w-full animate-message-appear mb-4", isUser ? "justify-start" : "justify-end")}>
      <div className={cn("max-w-[80%] rounded-2xl px-4 py-2 shadow-sm", isUser ? "bg-chat-user text-white" : "bg-chat-assistant text-gray-800")}>
        {isTyping ? <p className="text-sm md:text-base text-right" dir="rtl">
          ...
          h3: ({
            children
          }) => <h3 className="text-md font-bold mb-0.5 mt-0.5">{children}</h3>,
          ul: ({
            children
          }) => <ul className="mb-0.5 pr-2 list-disc space-y-0">{children}</ul>,
          ol: ({
            children
          ...
          td: ({
            children
          }) => <td className="px-1 py-0.5 border-gray-200">{children}</td>
        }}>
              {content}
            </ReactMarkdown>
          </div>}
